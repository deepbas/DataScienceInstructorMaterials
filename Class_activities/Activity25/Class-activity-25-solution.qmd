...
```{r}
pruned_tree <- prune(tree_fit$fit, cp=0.01)
...
```{r}
# Extract the ranger model from the fitted parsnip model
ranger_model <- rf_fit$fit

# Get the tree from the ranger model
library(ranger)
single_tree <- ranger::getTree(ranger_model, k = 1)

# Visualize the tree
plot(as.party(single_tree), type = "simple")
