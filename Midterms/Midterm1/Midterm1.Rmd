...
```{r}
data %>% mutate(decade = str_c(str_extract(year, "^\\d{3}"), 0)) %>% mutate(decade = factor(decade))
```
...
```{r, eval=FALSE}
ggplot(data_new, aes(x = _________, y = ________, color = _______)) +
  geom_point() +
...
stats_by_decade <- data_new %>%
  mutate(
    mean_price = mean(price, na.rm = TRUE, .by = decade),
    se_price = sd(price, na.rm = TRUE) / sqrt(n()), .by = decade) %>% 
  ungroup()
...
```{r, eval=FALSE}
stats_by_decade %>%
  _________(decade = _________(decade)) %>%
  ggplot(______(x = ___________, y = ____________)) +
  geom_ribbon(______(ymin = _______ - 2 * _______, ymax = _______ + 2 * _______), 
              fill = "_____", alpha = _______) +
  geom_line(____________) +
  geom_point(_________, size = _______) +
...
```{r, eval=TRUE, echo=FALSE}
stats_by_decade %>%
  mutate(decade = as.double(as.character(decade))) %>%
...
as.duration(interval(dd[1], dd[2]))/ddays(1) # method 1
time_length(interval(dd[1], dd[2]), unit = "days") # method 2
```


```{r}
dmy(dates) %>% interval(lag(.), .) / ddays(1)
time_length(dd[2]- dd[1], unit = "days")
```
...
```{r}
fct_relevel(experience, c("Senior", "Mid", "Entry"))
```
...
experience
prepend_level(experience)
```


```{r}
prepend_level <- function(levels_vector) {
  stringr::str_glue("Level: {levels_vector}")
}

prepend_level1 <- function(fctr){
  fct_relabel(fctr, ~ str_c("Level: ", .x))
}
prepend_level1(experience_reversed)
```
...
### (d) **(Bonus)** What does the following code chunk do? Here, assume `prepend_level` is the same function that you devised in part (c) and `experience_reversed` is the same object that you defined in part (b) above. (5 points)

```{r, echo=TRUE, eval=FALSE}
forcats::fct_relabel(experience, prepend_level)
```
