...
#### (e). Which one of False Positive (FP) or False Negative (FN) is more detrimental in this example? You may assume positive diabetes to be a positive case to answer this question. 
...
### Let's do a 10-fold cross validation across a grid of number of neighbors. The following is a plot of how the various metrics change when we vary `k`, the number of neighbors.  A corresponding table with the optimal number of neighbors for the different metrics is also given.
...
#### (e). Why do you think the sensitivity and the specificity grow in opposing pattern as the number of neighbors increases?
...
#### (a). The k-NN method of clustering will yield different accuracies as the value of k changes. As k approaches n, the sample size, what value will the accuracy of the method approach? Explain.
