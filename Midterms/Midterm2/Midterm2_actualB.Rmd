...
```{r, eval=FALSE, echo=FALSE, fig.pos = "H"}
...
ui <- fluidPage(
  titlePanel("Gapminder Data for Selected Country"),
  selectInput("countryInput", "Select a Country:", choices = unique(gapminder$country)),
  actionButton("showButton", "Show Latest Data"),
  htmlOutput("countryInfo")
)

server <- function(input, output) {
  # Use eventReactive to update the data when the button is clicked
  latestData <- eventReactive(input$showButton, {
  ...
  output$countryInfo <- renderUI({
    countryData <- latestData()
    HTML(stringr::str_c(
      "Country:", countryData$country, " ",
      "Life Expectancy:", round(countryData$lifeExp, 2), "years"
    ))
  })
}
