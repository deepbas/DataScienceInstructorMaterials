...
knitr::opts_chunk$set(collapse=TRUE, warning = FALSE, 
                      ...
                      fig.height = 4, fig.width = 5, error = TRUE, eval = FALSE)
...
bootstrap_ci(df_waste %>%  select(-1))
```
...
library(ggplot2)
library(dplyr)
library(babynames)  # Ensure this package is installed and loaded

ui <- fluidPage(
  titlePanel("Baby Names Trends"),
  sidebarLayout(
    sidebarPanel(
      selectInput("year", "Choose a year:", choices = unique(babynames$year)),
...
server <- function(input, output) {
  # Initialize reactive values to store the current selections
  values <- reactiveValues(year = NULL, gender = NULL)

  # Check for changes in the inputs and update reactive values accordingly
  observe({
    if (!is.null(input$year) && !is.null(input$gender)) {
      if (!identical(input$year, values$year) || !identical(input$gender, values$gender)) {
  ...
  # Render plot only when the update button is pressed
  observeEvent(input$update, {
    # Perform actions only if values are set
    if (!is.null(values$year) && !is.null(values$gender)) {
      # Proceed to filter data and generate the plot
      filtered_data <- babynames %>%
        filter(year == values$year, sex == values$gender) %>%
        arrange(desc(n)) %>%
        head(10)
...
server <- function(input, output) {
  # Reactive expression to check if both inputs have been set
  validInputs <- reactive({
    !is.null(input$year) && !is.null(input$gender)
  })
  ...
  }, ignoreInit = TRUE)  # Avoid triggering when the app initializes
}
...
```{r}
  ...
  updateData <- function() {
    variable_name <- paste(tolower(input$gender), input$year, sep = "_")
...
html <- read_html('http://localhost:8080')
